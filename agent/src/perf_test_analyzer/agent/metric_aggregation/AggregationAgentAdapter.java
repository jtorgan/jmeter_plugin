...
public class AggregationAgentAdapter extends AgentLifeCycleAdapter {
	...
	public void beforeBuildFinish(@NotNull AgentRunningBuild build, @NotNull BuildFinishedStatus buildStatus) {
		...
		if (!features.isEmpty()) {
			...
			logger.activityStarted(PerformanceLogger.PERFORMANCE_TESTS_ACTIVITY_NAME);
			...
			logger.activityFinished(PerformanceLogger.PERFORMANCE_TESTS_ACTIVITY_NAME);
			...
			if (properties.isCheckReferences()) {
				logger.activityStarted(PerformanceLogger.CHECK_REFERENCE_ACTIVITY_NAME);
				if (properties.isFileValues()) {
					FileValuesChecker checker = new FileValuesChecker(logger,
				...
				logger.activityFinished(PerformanceLogger.CHECK_REFERENCE_ACTIVITY_NAME);
			}
		}
	}

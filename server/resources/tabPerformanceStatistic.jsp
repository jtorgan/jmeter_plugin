...
<%--@elvariable id="performanceOKTests" type="java.util.Collection<perf_statistic.server.model.PerformanceTestRun>"--%>
<%--@elvariable id="performanceFailedTests" type="java.util.Collection<perf_statistic.server.model.PerformanceTestRun>"--%>
  ...
  <script type="text/javascript">
    $j(".test-collapse").unbind('click').bind('click', function (event) {
      ...
      if (!$j.trim($j(tcChartContainer).html())) {
        ...
        BS.ajaxUpdater($j(tcChartContainer).get(0), "/buildGraph.html?jsp=${teamcityPluginResourcesPath}perfChartsTC.jsp&buildTypeId=${build.buildTypeExternalId}&buildId=${build.buildId}&chartKey=" + testChartKey + "&testName=" + testName, {
          method: "get",
          evalScripts: true
        });
